...
CREATE VIEW dist_table_1_view AS
	SELECT count(*) AS cnt FROM dist_table_1;

WITH cte AS (
    SELECT int_col FROM dist_table_1 ORDER BY 1 LIMIT 3
),
unreferenced_cte AS (
    SELECT * FROM local_table_5
)
INSERT INTO local_table_5 (col_5, col_3)
SELECT 12, 'string_11'
FROM dist_table_1 t1
JOIN cte t2 ON (t1.dist_col < t2.int_col)
JOIN dist_table_1_view t3 ON (t1.dist_col != t3.cnt)
GROUP BY t1.text_col_1;
